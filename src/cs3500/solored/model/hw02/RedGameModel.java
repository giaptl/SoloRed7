...
/**
 * Behaviors for a solo game of RedSeven. The game consists of four structures:
 * <ul>
...
public interface RedGameModel<C extends Card> {
  ...
  /**
   * Play the given card from the hand to the losing palette chosen. The card is removed from the
   * hand and placed at the far right end of the palette.
   *
   ...
   * @throws IllegalArgumentException if cardIdxInHand < 0 or greater/equal to the number of cards
   *                                  in hand
  ...
  /**
   * Play the given card from the hand to the canvas. This changes the rules of the game for all
   * palettes. The method can only be called once per turn.
   *
   * @param cardIdxInHand a 0-index number representing the card to play from the hand
   * @throws IllegalStateException    if the game has not started or the game is over
   * @throws IllegalArgumentException if cardIdxInHand < 0 or greater/equal to the number of cards
  ...
  /**
   * Draws cards from the deck until the hand is full OR until the deck is empty, whichever occurs
   * first. Newly drawn cards are added to the end of the hand (far-right conventionally).
  ...
  /**
   * Starts the game with the given options. The deck given is used to set up the palettes and hand.
  ...
  /**
   * Returns a copy of the hand in the game. This means modifying the returned list or the cards in
   * the list has no effect on the game.
   *
   * @return a new list containing the cards in the player's hand in the same order as in the
  ...
  /**
   * Returns a copy of the specified palette. This means modifying the returned list or the cards in
   * the list has no effect on the game.
   *
   * @param paletteNum 0-based index of a particular palette
   * @return a new list containing the cards in specified palette in the same order as in the
   *         current state of the game.
   * @throws IllegalStateException    if the game has not started
   * @throws IllegalArgumentException if paletteIdx < 0 or more than the number of palettes
   */
  ...
  /**
   * Return the top card of the canvas. Modifying this card has no effect on the game.
   *
  ...
  /**
   * Get a NEW list of all cards that can be used to play the game. Editing this list should have no
   * effect on the game itself. Repeated calls to this method should produce a list of cards in the
   * same order. Modifying the cards in this list should have no effect on any returned list or the
   * game itself.
   *
